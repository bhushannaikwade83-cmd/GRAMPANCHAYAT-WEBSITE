    ...
    >
      <button
        ...
        style={{
          ...
          alignItems: "center",
          gap: "4px",
          color: items.some((item) =>
            isPathMatch(location.pathname, parentName, item)
          )
            ? "#2196f3"
            : "black",
          borderBottom: items.some((item) =>
            isPathMatch(location.pathname, parentName, item)
          )
            ? "2px solid #f32121ff"
            : "none",
          ...
          {items.map((item, index) => (
            <Link
              ...
              style={{
                display: "block",
                padding: "12px 20px",
                color: isPathMatch(location.pathname, parentName, item)
                  ? "#2196f3"
                  : "#333",
                background: isPathMatch(location.pathname, parentName, item)
                  ? "#f0f8ff"
                  : "transparent",
                transition: "all 0.2s ease",
                borderLeft: isPathMatch(location.pathname, parentName, item)
                  ? "4px solid #2196f3"
  ...
  const searchableItems = [
    ...navLinks.flatMap((link) =>
      link.dropdown
        ? link.dropdown.map((d) => ({
            name: `${link.name} - ${d}`,
            path: getLinkPath(link.name, d),
          }))
        : [{ name: link.name, path: link.to }]
    ),
  ];
            ...
            {navLinks.map((link, i) =>
              link.dropdown ? (
                <DropdownButton
                  ...
                  title={link.name}
                  anchor={
                    link.name === "ग्रामपंचायत"
                      ? grampanchayatAnchor
                      : link.name === "निर्देशिका"
                      ? directoryAnchor
                      : link.name === "उपक्रम"
                      ? upkramAnchor
                      : yojnaAnchor
                  ...
                  handleOpen={
                    link.name === "ग्रामपंचायत"
                      ? handleOpen(setGrampanchayatAnchor)
                      : link.name === "निर्देशिका"
                      ? handleOpen(setDirectoryAnchor)
                      : link.name === "उपक्रम"
                      ? handleOpen(setUpkramAnchor)
                      : handleOpen(setYojnaAnchor)
                  }
                  handleClose={
                    link.name === "ग्रामपंचायत"
                      ? handleClose(setGrampanchayatAnchor)
                      : link.name === "निर्देशिका"
                      ? handleClose(setDirectoryAnchor)
                      : link.name === "उपक्रम"
                      ? handleClose(setUpkramAnchor)
                      : handleClose(setYojnaAnchor)
                  }
                  items={link.dropdown}
                ...
                <Link
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    color:
                      location.pathname === link.to ? "white" : "black",
                    background:

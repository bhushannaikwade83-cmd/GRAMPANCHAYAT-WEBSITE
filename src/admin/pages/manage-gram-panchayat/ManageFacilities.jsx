...
const initialState = { name: '', type: 'Others', description: '', location: '', timings: '', imageURL: '', docURL: '' };
...
const ManageFacilities = () => {
  ...
  const colRef = useMemo(() => collection(db, 'facilities'), []);
  ...
  const handleSave = async () => {
    if (!current.name.trim()) {
      setNotification({ open: true, message: 'सुविधेचे नाव आवश्यक आहे.', severity: 'warning' });
      return;
    }
    try {
      const payload = { 
      ...
      if (isEditing) await updateDoc(doc(db, 'facilities', current.id), payload); else await addDoc(colRef, payload);
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteDoc(doc(db, 'facilities', id)); setNotification({ open: true, message: 'हटवले.', severity: 'success' }); fetchRows(); }
  ...
  const handleDocChange = async (e) => {
    try {
      ...
      fd.append('folder', 'grampanchayat/facilities_docs');
        ...
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 0 }}>
            <Grid item xs={12} md={6}>
              <TextField label="सुविधेचे नाव" fullWidth value={current.name} onChange={(e)=>setCurrent({ ...current, name: e.target.value })} />
            </Grid>
            <Grid item xs={12} md={6}>
            ...
            <Grid item xs={12} md={6}>
              <TextField label="कार्यरत वेळ (ऐच्छिक)" fullWidth value={current.timings} onChange={(e)=>setCurrent({ ...current, timings: e.target.value })} />
            </Grid>
            <Grid item xs={12} md={6}>

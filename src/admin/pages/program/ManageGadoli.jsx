...
const ManageGadoli = () => {
  ...
  // Firestore collection path: program/gadoli/items
  const gadoliCollection = collection(db, 'program', 'gadoli', 'items');

  useEffect(() => {
    const unsubscribe = onSnapshot(gadoliCollection, (snapshot) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem) {
        const docRef = doc(gadoliCollection, editingItem.id);
        await updateDoc(docRef, payload);
      } else {
        await addDoc(gadoliCollection, payload);
      }
      handleCloseDialog();
    } catch (err) {
      console.error('Failed to save gadoli item', err);
    }
  };

  const handleDelete = async (id) => {
    try {
      const docRef = doc(gadoliCollection, id);
      await deleteDoc(docRef);
    } catch (err) {
      console.error('Failed to delete gadoli item', err);
    }
  };
              ...
              <Table>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>कार्यक्रम प्रकार</TableCell>
                ...
                <TableBody>
                  {programs.map((program) => (
                    <TableRow key={program.id}>
                      ...
                      <TableCell>{program.activityType}</TableCell>

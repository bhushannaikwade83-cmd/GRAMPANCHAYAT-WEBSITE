...
const GramKridaSpardha = () => {
  ...
  useEffect(() => {
    const col = collection(db, 'program', 'kreedaspardha', 'items');
    ...
    <Box component="section" sx={{ py: 4 }}>
      <Container maxWidth="lg">
        <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 3 }}>
          <Box>
            <Typography variant="h4" sx={{ fontWeight: 700 }}>क्रीडा स्पर्धा</Typography>
            <Typography variant="body2" color="text.secondary">स्थानिक क्रीडा स्पर्धांची माहिती</Typography>
          </Box>
          <Chip color="primary" label={`${items.length} स्पर्धा`} variant="outlined" />
        </Stack>
        ...
        {!loading && items.length === 0 && (
          <Box sx={{ textAlign: 'center', py: 6, border: '1px dashed', borderColor: 'divider', borderRadius: 2 }}>
            <Typography variant="h6" sx={{ mb: 1 }}>सध्या कोणत्याही स्पर्धा उपलब्ध नाहीत</Typography>
            <Typography variant="body2" color="text.secondary">प्रशासन पॅनेल मधून स्पर्धा जोडा</Typography>
          </Box>
        )}
                  ...
                  <CardActionArea>
                    ...
                    <CardContent>
                      ...
                      <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
                        {it.sportType && <Chip size="small" variant="outlined" label={`क्रीडा: ${it.sportType}`} />}
        ...
        <Dialog open={!!preview} onClose={() => setPreview(null)} maxWidth="md" fullWidth>
          <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Typography variant="h6" noWrap>{preview?.title || 'क्रीडा स्पर्धा'}</Typography>
            ...
            {preview && (
              <Box>
                ...
                <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap" sx={{ mb: 2 }}>
                  {preview.sportType && <Chip size="small" label={`क्रीडा: ${preview.sportType}`} />}

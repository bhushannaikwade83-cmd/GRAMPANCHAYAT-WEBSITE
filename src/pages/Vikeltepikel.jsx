...
const Vikeltepikel = () => {
  ...
  useEffect(() => {
    const col = collection(db, 'program', 'vikeltepikel', 'items');
    ...
    <Box sx={{ py: 4 }}>
      <Container maxWidth="lg">
        <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 3 }}>
          <Box>
            <Typography variant="h4" component="h1" sx={{ fontWeight: 700 }}>विकल-ते-पिकेल</Typography>
            <Typography variant="body2" color="text.secondary">काय विकते तेच पिकते</Typography>
          </Box>
                  ...
                  <CardActionArea>
                    ...
                    <CardContent>
                      ...
                      <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
                        {it.productType && <Chip size="small" variant="outlined" label={`उत्पादन: ${it.productType}`} />}
                        ...
                        {it.price && <Chip size="small" variant="outlined" label={`किंमत: ₹${parseInt(String(it.price)).toLocaleString()}`} />}
                        {it.status && <Chip size="small" variant="outlined" label={`स्थिती: ${it.status === 'active' ? 'सक्रिय' : it.status === 'completed' ? 'पूर्ण' : 'प्रलंबित'}`} />}
                        {it.harvestDate && <Chip size="small" variant="outlined" label={`कापणी: ${it.harvestDate}`} />}
                        {it.marketDate && <Chip size="small" variant="outlined" label={`बाजार: ${it.marketDate}`} />}
        ...
        <Dialog open={!!preview} onClose={() => setPreview(null)} maxWidth="md" fullWidth>
          <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Typography variant="h6" noWrap>{preview?.title || 'विकल-ते-पिकेल'}</Typography>
            ...
            {preview && (
              <Box>
                ...
                <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap" sx={{ mb: 2 }}>
                  {preview.productType && <Chip size="small" label={`उत्पादन: ${preview.productType}`} />}
                  ...
                  {preview.price && <Chip size="small" label={`किंमत: ₹${parseInt(String(preview.price)).toLocaleString()}`} />}
                  {preview.status && <Chip size="small" label={`स्थिती: ${preview.status === 'active' ? 'सक्रिय' : preview.status === 'completed' ? 'पूर्ण' : 'प्रलंबित'}`} />}
                  {preview.harvestDate && <Chip size="small" label={`कापणी: ${preview.harvestDate}`} />}
                  {preview.marketDate && <Chip size="small" label={`बाजार: ${preview.marketDate}`} />}
